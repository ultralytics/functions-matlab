...
)
{
    ...
    for (n=0; n < MN; n++) {
        ...
        //if (fs<1 || s>ncols || ft<1 || t>nrows){
           ...
           //for (i = 0; i < ndim; i++) F[n+i*MN]=0;  } //put 0 if outside (already zeros!!)
        //else  {
        if (fs>=1 && s<=ncols && ft>=1 && t<=nrows){
            ndx = ft+(fs-1)*nrows;
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
    ...
    *newdims=M;newdims[1]=N; MN=M*N; vol=1;
    ...
    OUT_F = mxCreateNumericArray(ndim, newdims, mxINT16_CLASS, mxREAL);
  ...
  /* Assign pointers to the input/output arguments */
    Z      = (short*)mxGetPr(IN_Z);
    S      = (float*)mxGetPr(IN_S);
    T      = (float*)mxGetPr(IN_T);
    F      = (short*) mxGetPr(OUT_F);
