...
net = fitnet(hiddenLayers,'trainscg'); %trainlm, trainbr, trainscg
%net = patternnet(hiddenLayers,'trainscg'); %trainlm, trainbr, trainscg

net.trainParam.epochs=2E4;
net.trainParam.time=60*60*24; %max seconds to train
net.trainParam.max_fail = 500; %validation checks
% 
...
for i=1:iterations
    switch net.name
        ...
        otherwise %function fitting
            [neta,tra] = train(net,x,t,[],[],ew,'UseParallel','yes');  disp(perform(neta,t,neta(x)));  tr=tra;
            %neta.trainParam.max_fail=2;  for j=1:2;  [neta,tr]=train(neta,x,t,[],[],ew);  disp(perform(neta,t,neta(x))); end; 
    end
    isigma=std(neta(x)'-t');
    s=sprintf('%.3f, ',isigma);  fprintf(['Network %g: [' s(1:end-2) '] sigma, %.3g perf in %.0fs\n'],i,tr.best_perf,tra.time(end));
